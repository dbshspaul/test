			...
			<div class="art-layout-cell art-content">
				...
				<div style="margin-left: 25px; margin-top: 25px; margin-right: 25px">
					...
					<table>
						<tr>
							...
							<td><div>
									...
									<c:if test="${p.country !=1}"></c:if>
										...
										<c:if test="${p.country !=1}">
											<tr>
												<td>Country:</td>
												<td>${userOb.country}</td>
											</tr>
										</c:if>
										<c:if test="${p.contactNo!=1}">
											<tr>
												<td>Contact number:</td>
												<td>${userProObj.contactNo}</td>
											</tr>
										</c:if>
										<c:if test="${p.address!=1}">
											<tr>
												<td>Address:</td>
												<td>${userProObj.address}</td>
											</tr>
										</c:if>
										<c:if test="${p.languagesKnown !=1}">										
											<tr>
												<td>Languages Known</td>
												<td>${userProObj.languagesKnown}</td>
											</tr>
										</c:if>
										<c:if test="${p.education !=1}">
											<tr>
												<td>Education:</td>
												<td>${userProObj.education}</td>
											</tr>
										</c:if>
										<c:if test="${p.hobbies !=1}">
											<tr>
												<td>Hobbies:</td>
												<td>${userProObj.hobbies}</td>
											</tr>
										</c:if>
										<c:if test="${p.aboutUser !=1}">
											<tr>
												<td>About:</td>
						...
						<tr>
							<td><a href="friendWall?friendid=${userOb.emailId}">view
									wall</a></td>
							<td><a href="chat?frndId=${userOb.emailId}">Chat</a></td>
							<td><a href="gallery?friendId=${userOb.emailId}">Gallery</a></td>
						</tr>
					</table>
				</div>
			</div>
